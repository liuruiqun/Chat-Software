		...
		case P2PMESSAGE:
			{
				...
				{
					//printf("Recv a Message, input 'Y' to show or 'N' to ignore.\n");
...
int main(int argc, char* argv[])
{
	try
	{
		...
		PrimaryUDP = mksock(SOCK_DGRAM);//申请套接字
		BindSock(PrimaryUDP);			//绑定套接子

		cout<<"Please input server ip:";//输入服务器地址
		cin>>ServerIP;

		cout<<"Please input your name:";//输入用户呢称
		cin>>UserName;

		ConnectToServer(PrimaryUDP, UserName, ServerIP);//连接服务器

		HANDLE threadhandle = CreateThread(NULL, 0, RecvThreadProc, NULL, NULL, NULL);//多线程并发通信
