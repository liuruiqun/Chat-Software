...
int main(int argc, char* argv[])
{
	try{
		...
		sockaddr_in local;				//确定并填写端口地址
		local.sin_family=AF_INET;
		local.sin_port= htons(SERVER_PORT); 
		local.sin_addr.s_addr = htonl(INADDR_ANY);
		int nResult=bind(PrimaryUDP,(sockaddr*)&local,sizeof(sockaddr));		//建立套接字与端口的联系
		...
		for(;;)
		{
			...
			int ret = recvfrom(PrimaryUDP, (char *)&recvbuf, sizeof(stMessage), 0, (sockaddr *)&sender, &dwSender);//接收信息
			if(ret <= 0)
			{
				printf("recv error");
				continue;
			...
			{
				int messageType = recvbuf.iMessageType;		//判断信息类型并作出响应
				switch(messageType){
				case LOGIN:
					{
						//  将这个用户的信息记录到用户列表中
